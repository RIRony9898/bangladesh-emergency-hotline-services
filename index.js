...
cards.forEach((card) => {
  ...
  copyBtn.addEventListener("click", () => {
    navigator.clipboard.writeText(phoneNumber).then(() => {
      ...
      toast.className =
        "absolute bg-green-500 text-white px-2 py-1 rounded shadow text-sm";
  ...
  callBtn.addEventListener("click", () => {
    if (points < 20) {
      alert(
        "❌You don’t have enough coins; you will need 20 points to make a call!"
      );
      return;
    }

    const confirmCall = confirm(`📞 Do you want to call this number: ${phoneNumber}?`);
    if (confirmCall) {
      points -= 20;
  ...
  <p class="font-semibold text-lg mb-1 whitespace-normal ">${
